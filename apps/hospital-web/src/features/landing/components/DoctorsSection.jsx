...
import { CalendarCheck, ArrowRight, Award, Sparkles, Stethoscope } from 'lucide-react';
...
const DOCTORS_HUMAN = [
  ...
  },
  {
    ...
    role: 'Chuyên khoa Ung Bướu & Nhi Khoa',
        ...
        <Reveal variant="up" className="mb-14 flex flex-col items-start justify-between gap-4 md:flex-row md:items-end">
          <div>
            <span className="inline-flex items-center gap-2 rounded-full bg-sky-50 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-sky-700 mb-3 border border-sky-100 shadow-xs">
              ...
              <span>Đội Ngu Y Bác Sĩ Đầu Ngành</span>
            </span>
            <h2 id="doctors-heading" className="text-3xl font-extrabold text-slate-900 sm:text-4xl tracking-tight">
              Hội Đồng Bác Sĩ Chuyên Gia
            </h2>
            <p className="mt-2 text-sm text-slate-500 max-w-lg font-medium">
              Đội ngũ Giáo sư, Tiến sĩ, Bác sĩ chuyên khoa II giàu kinh nghiệm luôn sẵn sàng tư vấn và thăm khám cho bạn.
            </p>
          </div>
          <a
            href="#lien-he"
            className="inline-flex items-center gap-2 rounded-2xl border border-sky-200 bg-sky-50 px-5 py-3 text-xs font-bold text-sky-700 hover:bg-sky-600 hover:text-white transition-all shadow-xs"
          >
            <Stethoscope className="w-4 h-4" />
            <span>Đăng ký hẹn khám bác sĩ</span>
        ...
        <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {DOCTORS_HUMAN.map((doctor, index) => (
            <Reveal key={doctor.name} variant="up" delay={index * 80}>
              <article className="group flex h-full flex-col justify-between overflow-hidden rounded-3xl border border-slate-200/80 bg-white p-4 shadow-sm hover:shadow-xl transition-all duration-300">
                <div>
                  <div className="relative mb-4 aspect-[4/5] overflow-hidden rounded-2xl bg-slate-100">
                    <img
                      ...
                      loading="lazy"
                    />
                    <div className="absolute top-3 left-3 rounded-full bg-white/95 backdrop-blur-xs px-3 py-1 text-[11px] font-bold text-slate-800 shadow-xs border border-slate-200 flex items-center gap-1">
                ...
                <div className="pt-4 px-1">
                  <a
                    href="#lien-he"
                    className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-50 py-2.5 text-xs font-bold text-slate-700 hover:bg-sky-600 hover:text-white transition-all shadow-xs"
                  >
