...
export class EntityRecoveryService {
  ...
  private async evaluateRow(
    ...
    if (!lightVerification.ok) {
      const cluster = await this.resolveClusterInfo(entity, row.entityId, null, row);
      return {
        ...base,
        ...cluster,
    ...
    const cluster = await this.resolveClusterInfo(entity, row.entityId, liveSnapshot, row);
