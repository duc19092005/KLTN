...
/**
 * Audit hashing utilities for tamper-evidence (V2 Canonical Standard).
 */
...
/** Compute deterministic record hash for DB entity tables storing hash256 & dataSalt */
export function computeRecordHash(
  ...
  return createHash('sha256')
    .update(canonicalize(data) + salt + pepper)
    .digest('hex');
}

/** Legacy V1 entry hash for compatibility */
export function computeEntryHash(
  ...
  return createHash('sha256')
    .update(
      `${core.seq}|${prevHash}|${core.actorId ?? ''}|${core.action}|${core.entity}|${core.entityId ?? ''}|${core.dataHash ?? ''}|${core.createdAtIso}|${pepper}`,
    )
    .digest('hex');
}
