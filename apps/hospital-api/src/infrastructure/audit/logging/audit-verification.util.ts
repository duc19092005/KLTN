...
import { buildAuditEncryptionAad, decryptAuditSnapshot, EncryptedAuditSnapshot } from '../crypto/audit-encryption.util';
 ...
 * on the row without decrypting snapshots.
 */
  ...
  if (row.hashVersion === AUDIT_ENTRY_V2) {
    ...
    if (suspiciousFields.length > 0) {
      return {
        ...
        status: 'PENDING',
        version: 'V2',
        reason: `Thiếu field bắt buộc để kiểm tra hash: ${suspiciousFields.join(', ')}.`,
        suspiciousFields,
      };
    }
