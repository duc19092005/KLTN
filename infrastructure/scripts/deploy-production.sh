...
COMPOSE_FILE="${COMPOSE_FILE:-$REPOSITORY_ROOT/infrastructure/compose/compose.prod.yml}"
ENV_FILE="${ENV_FILE:-$REPOSITORY_ROOT/.env}"
BACKEND_ENV_FILE="${BACKEND_ENV_FILE:-$REPOSITORY_ROOT/apps/hospital-api/.env}"
LEGACY_BACKEND_ENV_FILE="${LEGACY_BACKEND_ENV_FILE:-$REPOSITORY_ROOT/backend/.env}"
...
current_image() {
  local service="$1"
  docker compose --env-file "$ENV_FILE" -f "$COMPOSE_FILE" ps -q "$service" 2>/dev/null \
...
migrate_legacy_backend_env() {
  ...
  if [[ -f "$LEGACY_BACKEND_ENV_FILE" ]]; then
    log "Migrating legacy backend env to ${BACKEND_ENV_FILE}"
    install -d -m 700 "$(dirname "$BACKEND_ENV_FILE")"
    mv "$LEGACY_BACKEND_ENV_FILE" "$BACKEND_ENV_FILE"
    chmod 600 "$BACKEND_ENV_FILE"
    rmdir "$(dirname "$LEGACY_BACKEND_ENV_FILE")" 2>/dev/null || true
  fi
}
