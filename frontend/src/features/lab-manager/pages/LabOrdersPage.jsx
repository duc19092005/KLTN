...
export default function LabOrdersPage() {
  ...
  const openResultFile = async (fileId) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || 'Không mở được tệp kết quả');
    }
  };
  ...
  return (
    <DashboardLayout user={user} navItems={LAB_MANAGER_NAV_ITEMS} activeItem="orders" onNavigate={(id) => navigate(labManagerRouteFor(id))} onLogout={logout}>
      ...
      {activeOrder && <OrderModal order={activeOrder} form={form} setForm={setForm} onStatus={updateStatus} onSubmit={submitResult} onOpenFile={openResultFile} busy={busy} onClose={() => setActiveOrder(null)} />}
    </DashboardLayout>
  );
}
...
function ResultArchive({ order, onOpenFile }) { return <div className="rounded-2xl border border-slate-100 bg-slate-50 p-5"><h3 className="mb-4 text-lg font-black text-slate-950">Kết quả</h3>{order.results?.length ? <div className="space-y-3">{order.results.map((r, i) => <div key={r.id || i} className="rounded-2xl border border-slate-100 bg-white p-4">{r.note && <p className="mb-3 text-sm font-semibold text-slate-700">{r.note}</p>}<div className="flex flex-wrap gap-2">{r.files?.map((f, j) => <button key={j} type="button" onClick={() => onOpenFile(f.id)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-700 hover:bg-slate-100">{f.originalName || 'Xem tệp'}</button>)}</div></div>)}</div> : <p className="text-sm font-semibold text-slate-500">Chưa có tệp</p>}</div>; }
