      ...
      <div className="max-w-7xl mx-auto space-y-6">
        <section className="relative overflow-hidden rounded-[28px] border border-blue-100 bg-gradient-to-br from-white via-blue-50 to-emerald-50 p-7 shadow-sm">
          ...
          <div className="relative flex flex-col lg:flex-row lg:items-end justify-between gap-5">
            <div>
              <p className="text-[11px] uppercase tracking-[0.24em] font-black text-blue-600">Doctor Queue Workspace</p>
              <h1 className="mt-2 text-3xl font-black text-slate-950 tracking-tight">Hàng đợi khám</h1>
              <p className="mt-2 max-w-2xl text-sm text-slate-600">Page riêng cho bác sĩ xử lý bệnh nhân: chọn lượt khám, xem phiếu chi tiết và cập nhật tiến trình lâm sàng.</p>
            </div>
            <button id="doctor-refresh-queue-button" onClick={loadVisits} className="h-11 px-4 rounded-2xl border border-blue-100 bg-white text-sm font-black text-blue-700 hover:bg-blue-50 transition-all">
              {loading ? <LoadingIndicator size="sm" /> : 'Làm mới hàng đợi'}
            </button>
          </div>
        </section>
        ...
        <section className="grid grid-cols-1 xl:grid-cols-12 gap-6 items-start">
          <aside className="xl:col-span-5 rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden">
            <div className="p-5 border-b border-slate-100">
              <div className="relative">
                <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400">⌕</span>
                <input id="doctor-visit-search-input" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Tìm theo tên, mã BN, SĐT, CCCD..." className="w-full rounded-2xl border border-slate-200 bg-slate-50 py-3 pl-10 pr-4 text-sm font-semibold outline-none transition-all focus:bg-white focus:border-blue-400 focus:ring-4 focus:ring-blue-100" />
              </div>
              <div className="mt-4 flex gap-2 overflow-x-auto pb-1">
                {FILTERS.map((item) => <button key={item.id || 'ALL'} onClick={() => setFilter(item.id)} className={`px-3 py-2 rounded-xl text-xs font-black border whitespace-nowrap transition-all ${filter === item.id ? 'bg-blue-600 text-white border-blue-600 shadow-lg shadow-blue-100' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>{item.label}</button>)}
              </div>
            </div>
            <div className="p-4 space-y-3 max-h-[720px] overflow-y-auto">
              ...
              {!loading && filteredVisits.map((visit) => <VisitCard key={visit.id} visit={visit} active={activeVisit?.id === visit.id} onClick={() => setActiveVisit(visit)} />)}
            </div>
          </aside>

          <main className="xl:col-span-7">
            {!activeVisit ? <EmptyDetail /> : <VisitDetail visit={activeVisit} updating={updatingId === activeVisit.id} onStatus={(status) => updateStatus(activeVisit, status)} />}
          </main>
        </section>
      </div>
    </DashboardLayout>
...
function EmptyDetail() { return <div className="min-h-[560px] rounded-3xl border border-dashed border-slate-300 bg-white/80 flex flex-col items-center justify-center text-center p-8"><div className="mb-4 flex h-16 w-16 items-center justify-center rounded-3xl bg-blue-50 text-3xl text-blue-600">🩺</div><h2 className="text-xl font-black text-slate-950">Chọn một lượt khám</h2><p className="mt-2 max-w-sm text-sm text-slate-500">Thông tin bệnh nhân, triệu chứng, phòng khám và nút cập nhật trạng thái sẽ hiển thị tại đây.</p></div>; }
function EmptyQueue() { return <div className="rounded-3xl border border-dashed border-slate-200 bg-slate-50 p-10 text-center"><div className="text-4xl">🌿</div><h3 className="mt-3 font-black text-slate-800">Không có lượt khám phù hợp</h3><p className="mt-1 text-sm text-slate-500">Thử đổi bộ lọc hoặc làm mới hàng đợi.</p></div>; }
