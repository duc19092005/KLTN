      ...
      <div className="flex max-h-[92vh] w-full max-w-4xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
        ...
        <div className="shrink-0 border-b border-slate-100 p-6">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[11px] font-black uppercase tracking-[0.18em] text-indigo-600">Blockchain Audit</p>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            <SummaryChip tone="total" value={verifyResult?.total ?? '—'} label="Tổng bác sĩ" />
...
function VerifyList({ items }) {
  if (!items.length) return <Empty title="Chưa có bác sĩ" desc="Tạo bác sĩ để bắt đầu neo dữ liệu lên blockchain." />;
  return (
        ...
        return (
          <div key={item.id} className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm space-y-4">
            <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
              <div>
                <h4 className="text-base font-black text-slate-950">Bác sĩ: {item.licenseNumber || 'N/A'}</h4>
                <p className="text-xs text-slate-500">Chuyên khoa: <span className="font-bold text-slate-700">{item.specialty || 'N/A'}</span></p>
              </div>
              <span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[10px] font-black ${tone.cls}`}>
                <span className={`h-1 w-1 rounded-full ${tone.dot}`} />
                {tone.label}
              </span>
            </div>

            <div className="text-[11px] pt-3 border-t border-slate-100">
              <div className="space-y-1.5 bg-slate-50/50 p-2.5 rounded-xl border border-slate-100">
                <HashRow label="Hash tính lại" value={item.recomputedHash} match={item.dbMatches} />
                <HashRow label="Hash trong CSDL" value={item.storedHash} match={item.dbMatches} />
...
function HistoryList({ items }) {
  if (!items.length) return <Empty title="Chưa có lịch sử" desc="Mọi thay đổi bác sĩ sẽ được ghi lại và neo lên blockchain." />;
  return (
    <div className="space-y-3">
      {items.map((log) => (
          ...
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-black text-slate-900">
                {log.afterJson?.licenseNumber || log.beforeJson?.licenseNumber || log.entityId}
              </span>
            </div>
