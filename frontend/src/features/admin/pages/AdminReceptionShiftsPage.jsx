...
function ShiftTab({ kind, service, toast }) {
  ...
  return <>
    ...
    <section className="flex flex-col gap-3 rounded-2xl border border-slate-100 bg-white p-4 shadow-sm sm:flex-row sm:items-center">
    ...
    {loading ? <LoadingIndicator size="lg" label="Đang tải dữ liệu..." /> : <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
      ...
      {shifts.length === 0 ? <p className="p-8 text-center text-sm font-bold text-slate-400">Chưa có ca nào.</p> : <div className="overflow-x-auto"><table className="ui-table w-full text-left text-sm"><thead className="bg-slate-50 text-[10px] font-black uppercase tracking-wider text-slate-500"><tr><th className="px-4 py-3 text-left">Nhân viên</th><th className="px-4 py-3 text-left">Phòng ban</th><th className="px-4 py-3 text-left">Ngày</th><th className="px-4 py-3 text-left">Ca</th><th className="px-4 py-3 text-left">Trạng thái</th><th className="px-4 py-3 text-right">Thao tác</th></tr></thead><tbody className="divide-y divide-slate-100">{shifts.map((s) => { const meta = STATUS_META[s.status] || STATUS_META.PENDING; return <tr key={s.id} className="hover:bg-slate-50"><td className="px-4 py-3 font-bold text-slate-800">{s.staff?.fullName || 'N/A'}</td><td className="px-4 py-3 text-slate-600">{deptLabel(s)}</td><td className="px-4 py-3 text-slate-600">{dateLabel(s)}</td><td className="px-4 py-3 text-slate-600">{SHIFT_LABEL[s.shiftCode] || s.shiftCode}</td><td className="px-4 py-3"><span className={`inline-flex items-center gap-1.5 rounded-full border px-2.5 py-0.5 text-[11px] font-black ${meta.bg}`}><span className={`h-1.5 w-1.5 rounded-full ${meta.dot}`} />{meta.label}</span></td><td className="px-4 py-3 text-right">{s.status === 'PENDING' ? <div className="inline-flex gap-2"><button onClick={() => approve(s.id)} className="rounded-lg bg-cyan-600 px-3 py-1 text-[11px] font-black text-white hover:bg-cyan-700">Duyệt</button><button onClick={() => setRejectTarget(s)} className="rounded-lg border border-rose-200 px-3 py-1 text-[11px] font-black text-rose-600 hover:bg-rose-50">Từ chối</button></div> : <span className="text-[11px] font-semibold text-slate-400">—</span>}</td></tr>; })}</tbody></table></div>}
    </section>}
