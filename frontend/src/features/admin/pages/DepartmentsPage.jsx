...
const emptyForm = { departmentCode: '', name: '', floor: '', status: 'ACTIVE', type: 'LABORATORY', canReceiveOrders: false, description: '', specialty: '' };
...
function canDepartmentReceiveOrders(type) { return ['LABORATORY', 'IMAGING', 'PHARMACY'].includes(type); }
...
export default function DepartmentsPage() {
  ...
  const submitDepartment = async (event) => {
    ...
    const payload = {
      ...
      specialty: ['CLINICAL', 'LABORATORY', 'IMAGING'].includes(form.type) ? (form.specialty || undefined) : undefined,
    };
    setBusy(true);
    try {
...
function DepartmentHistory({ departmentId }) {
  ...
  const actionLabel = {
    ...
    UPDATE: { label: 'Cập nhật', cls: 'bg-blue-50 text-blue-700 border-blue-100' },
    ...
    <div className="flex items-center justify-between border-t border-slate-100 p-4">
      ...
      <div className="flex gap-2">
        <button
          ...
          className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-black text-slate-600 hover:bg-blue-50 hover:text-blue-600 disabled:opacity-50"
        >
          Trước
        </button>
        <button
