...
const emptyRoom = { roomCode: '', roomName: '', doctorId: '', floor: '', description: '', status: 'ACTIVE' };
...
export default function ClinicalRoomsPage() {
  ...
  const load = async (page = pagination.page) => {
    ...
    try {
      const [roomRes, doctorRes] = await Promise.all([
        clinicalRoomService.search({ ...filters, page, limit: pagination.limit }),
        doctorService.search({ limit: 100 }),
      ]);
      ...
      setDoctors(getItems(doctorRes.data));
  ...
  const openEdit = (room) => { setEditing(room); setForm({ roomCode: room.roomCode || '', roomName: room.roomName || '', doctorId: room.doctorId || '', floor: room.floor || '', description: room.description || '', status: room.status || 'ACTIVE' }); setIsOpen(true); };
  const close = () => { setIsOpen(false); setEditing(null); setForm(emptyRoom); };
  const submit = async (event) => {
    event.preventDefault(); setBusy(true);
    try {
      const payload = { ...form, doctorId: form.doctorId || null };
      if (editing) await clinicalRoomService.update(editing.id, payload);
      else await clinicalRoomService.create(payload);
  ...
  const remove = async (room) => {
    ...
    setBusy(true);
    try { await clinicalRoomService.remove(room.id); await load(pagination.page); }
    catch (err) { setError(getError(err)); }
    finally { setBusy(false); }
  };
  const search = async (event) => { event.preventDefault(); await load(1); };

  return <DashboardLayout user={user} navItems={ADMIN_NAV_ITEMS} activeItem="clinicalRooms" onNavigate={(id) => navigateAdmin(navigate, id)} onLogout={logout}><div className="max-w-7xl mx-auto space-y-6"><Hero onCreate={openCreate} totalLabel={totalLabel} />{error && <Alert>{error}</Alert>}{loading ? <LoadingIndicator size="lg" label="Đang tải phòng khám..." /> : <><SearchBar filters={filters} setFilters={setFilters} onSearch={search} /><section className="rounded-3xl border border-slate-100 bg-white shadow-sm overflow-hidden"><div className="p-5 border-b border-slate-100"><h3 className="text-xl font-black text-slate-950">Danh sách phòng khám</h3><p className="text-sm text-slate-500">Quản lý phòng khám và bác sĩ phụ trách.</p></div><div className="divide-y divide-slate-100">{rooms.map((room) => <RoomRow key={room.id} room={room} onEdit={openEdit} onRemove={remove} busy={busy} />)}{!rooms.length && <div className="p-6 text-center text-sm text-slate-500">Chưa có phòng khám.</div>}</div><Pagination pagination={pagination} onPageChange={load} /></section></>}{isOpen && <RoomModal form={form} setForm={setForm} doctors={doctors} onSubmit={submit} onClose={close} busy={busy} editing={editing} />}</div></DashboardLayout>;
}
function Hero({ onCreate, totalLabel }) { return <section className="rounded-[28px] border border-cyan-100 bg-gradient-to-br from-white via-cyan-50 to-emerald-50 p-8 shadow-sm flex flex-col lg:flex-row lg:items-end justify-between gap-5"><div><p className="text-[11px] font-black text-cyan-600 uppercase tracking-[0.24em] mb-3">Clinical Room OS</p><h2 className="text-3xl sm:text-4xl font-black text-slate-950 tracking-tight">Quản lý phòng khám</h2><p className="mt-3 text-sm sm:text-base text-slate-600">Tạo phòng khám, gán bác sĩ phụ trách và kiểm soát trạng thái hoạt động.</p><span className="mt-4 inline-flex rounded-xl bg-white/80 px-3 py-1 text-xs font-black text-cyan-700 border border-cyan-100">{totalLabel}</span></div><button onClick={onCreate} className="rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-black text-white shadow-lg shadow-cyan-100 hover:bg-cyan-700">+ Thêm phòng khám</button></section>; }
...
function RoomRow({ room, onEdit, onRemove, busy }) { return <article className="p-5 hover:bg-slate-50/70"><div className="grid grid-cols-1 lg:grid-cols-[0.8fr_1fr_1fr_0.6fr_180px] gap-4 lg:items-center"><Info label="Mã phòng" value={room.roomCode} /><Info label="Tên phòng" value={room.roomName} /><Info label="Bác sĩ" value={room.doctor?.staffProfile?.fullName || 'Chưa gán'} /><span className="w-fit rounded-lg border border-emerald-100 bg-emerald-50 px-2.5 py-1 text-xs font-black text-emerald-700">{room.status}</span><div className="flex gap-2 lg:justify-end"><SmallButton onClick={() => onEdit(room)} disabled={busy}>Sửa</SmallButton><SmallButton danger onClick={() => onRemove(room)} disabled={busy}>Xóa</SmallButton></div></div></article>; }
function RoomModal({ form, setForm, doctors, onSubmit, onClose, busy, editing }) { return <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm"><form onSubmit={onSubmit} className="w-full max-w-2xl rounded-3xl bg-white p-6 shadow-2xl space-y-4"><div className="flex justify-between gap-4"><div><p className="text-[11px] font-black text-cyan-600 uppercase tracking-[0.18em]">Clinical setup</p><h3 className="text-2xl font-black text-slate-950">{editing ? 'Cập nhật phòng khám' : 'Thêm phòng khám'}</h3></div><button type="button" onClick={onClose} className="rounded-xl border border-slate-200 px-3 py-2 text-sm font-black text-slate-500">Đóng</button></div><div className="grid grid-cols-1 md:grid-cols-2 gap-4"><Input label="Mã phòng" value={form.roomCode} onChange={(v) => setForm({ ...form, roomCode: v })} required /><Input label="Tên phòng" value={form.roomName} onChange={(v) => setForm({ ...form, roomName: v })} required /><Input label="Tầng" value={form.floor} onChange={(v) => setForm({ ...form, floor: v })} /><Select label="Trạng thái" value={form.status} onChange={(v) => setForm({ ...form, status: v })} options={[{ value: 'ACTIVE', label: 'ACTIVE' }, { value: 'INACTIVE', label: 'INACTIVE' }]} /><Select label="Bác sĩ phụ trách" value={form.doctorId} onChange={(v) => setForm({ ...form, doctorId: v })} empty="Chưa gán bác sĩ" options={doctors.map((d) => ({ value: d.id, label: `${d.staffProfile?.fullName || 'Bác sĩ'} - ${d.specialty}` }))} /><Input label="Mô tả" value={form.description} onChange={(v) => setForm({ ...form, description: v })} /></div><button disabled={busy} className="w-full rounded-2xl bg-cyan-600 px-5 py-3 text-sm font-black text-white hover:bg-cyan-700 disabled:opacity-70">{editing ? 'Lưu thay đổi' : 'Tạo phòng khám'}</button></form></div>; }
