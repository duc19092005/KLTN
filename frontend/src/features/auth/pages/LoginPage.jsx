...
export default function LoginPage() {
  ...
  const handleWalletLogin = async () => {
    setError('');
    if (!window.ethereum) { setError('MetaMask is required for admin wallet login.'); return; }
    try {
      setBusy(true);
    ...
    } catch (err) { setError(err.response?.data?.message || err.message || 'Xác thực ví thất bại.'); }
  ...
  const handleInviteLogin = async (event) => {
    ...
    try {
      ...
      if (!result.success) throw new Error(result.error);
      navigate('/authenticate', { replace: true });
    } catch (err) { setError(err.response?.data?.message || err.message || 'Mã mời không hợp lệ hoặc đã hết hạn.'); }
    finally { setBusy(false); }
  };
