...
export default function PatientHomePage() {
  ...
  return (
    <main className="min-h-screen bg-gradient-to-br from-cyan-50 via-white to-blue-50 px-4 py-10">
      <section className="mx-auto max-w-5xl overflow-hidden rounded-[2rem] border border-white/80 bg-white/90 shadow-2xl shadow-cyan-100/60 backdrop-blur">
        <div className="bg-gradient-to-br from-cyan-600 via-blue-600 to-indigo-700 p-8 text-white">
        ...
        <div className="grid gap-6 p-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className="space-y-4 rounded-3xl border border-slate-100 bg-slate-50/70 p-5">
            ...
            {error && <p className="rounded-xl border border-red-100 bg-red-50 px-3 py-2 text-xs font-bold text-red-700">{error}</p>}

            <button
              type="button"
              onClick={() => resolveHistory()}
              disabled={loading}
              className="w-full rounded-2xl bg-slate-950 px-5 py-3 text-sm font-black text-white shadow-lg shadow-slate-200 transition hover:bg-slate-800 disabled:opacity-60"
            >
              ...
              {visits.map((visit) => (
                <article key={visit.id || visit.visitCode} className="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${visit.integrityStatus === 'TAMPERED' ? 'bg-red-50 text-red-700 border border-red-100' : 'bg-emerald-50 text-emerald-700 border border-emerald-100'}`}>
