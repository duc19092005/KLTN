...
function StatCard({ label, value }) { return <article className="rounded-3xl border border-slate-100 bg-white p-4 shadow-sm"><p className="text-xs font-bold text-slate-500">{label}</p><strong className="mt-2 block text-3xl font-black text-slate-950">{String(value).padStart(2, '0')}</strong></article>; }
function ChartCard({ title, children }) { return <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><h2 className="text-lg font-black text-slate-950 mb-4">{title}</h2>{children}</section>; }
