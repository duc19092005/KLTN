  ...
  return (
    <DashboardLayout user={user} navItems={FRONTDESK_NAV_ITEMS} activeItem="patient-records" onNavigate={(id) => navigate(frontdeskRouteFor(id))} onLogout={logout}>
      <div className="mx-auto max-w-[1600px] space-y-4">
        ...
        {loading ? <LoadingIndicator size="lg" label="Đang tải hồ sơ..." /> : <section className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm"><div className="flex items-center justify-between border-b border-slate-100 px-5 py-4"><div><h2 className="text-lg font-black text-slate-950">Danh sách bệnh nhân</h2><p className="mt-1 text-xs font-semibold text-slate-500">{filteredPatients.length}/{patients.length}</p></div></div>{pagedPatients.length ? <><div className="hidden overflow-x-auto lg:block"><table className="ui-table min-w-full text-left"><thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500"><tr><th className="px-5 py-3">Bệnh nhân</th><th className="px-5 py-3">CCCD</th><th className="px-5 py-3">SĐT</th><th className="px-5 py-3">Lượt khám</th><th className="px-5 py-3">Gần nhất</th><th className="px-5 py-3 text-right">Thao tác</th></tr></thead><tbody className="divide-y divide-slate-100">{pagedPatients.map((patient) => <PatientRow key={patient.id} patient={patient} visits={visitsOf(patient)} onOpen={() => setSelectedPatient(patient)} />)}</tbody></table></div><div className="space-y-3 p-4 lg:hidden">{pagedPatients.map((patient) => <PatientCard key={patient.id} patient={patient} visits={visitsOf(patient)} onOpen={() => setSelectedPatient(patient)} />)}</div><Pagination page={page} totalPages={totalPages} total={filteredPatients.length} onPrev={() => setPage((v) => Math.max(1, v - 1))} onNext={() => setPage((v) => Math.min(totalPages, v + 1))} /></> : <Empty title="Không có dữ liệu" />}</section>}
      </div>
      {selectedPatient && <PatientDetailModal patient={selectedPatient} visits={visitsOf(selectedPatient)} onClose={() => setSelectedPatient(null)} onCreateVisit={() => navigate('/receptionist/intake', { state: { patient: selectedPatient } })} />}
    </DashboardLayout>
  );
}

function PatientRow({ patient, visits, onOpen }) { const latestVisit = visits[0]; const latestStatus = latestVisit ? getVisitStatus(latestVisit.status) : null; return <tr className="bg-white hover:bg-cyan-50/40"><td className="px-5 py-4"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-cyan-50 text-xs font-black text-cyan-700">{patient.fullName?.slice(0, 2).toUpperCase()}</div><div><p className="text-sm font-black text-slate-900">{patient.fullName}</p><p className="text-[11px] font-semibold text-slate-500">{patient.patientCode}</p></div></div></td><td className="px-5 py-4 text-xs font-bold text-slate-600">{patient.citizenId || '—'}</td><td className="px-5 py-4 text-xs font-bold text-slate-600">{patient.phone || '—'}</td><td className="px-5 py-4 text-xs font-black text-cyan-700">{visits.length}</td><td className="px-5 py-4">{latestStatus ? <StatusBadge st={latestStatus} /> : <span className="text-xs font-bold text-slate-400">Chưa có</span>}</td><td className="px-5 py-4 text-right"><button type="button" onClick={onOpen} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50">Chi tiết</button></td></tr>; }
function PatientCard({ patient, visits, onOpen }) { const latestVisit = visits[0]; const latestStatus = latestVisit ? getVisitStatus(latestVisit.status) : null; return <article className="w-full rounded-2xl border border-slate-100 bg-white p-4 text-left hover:bg-cyan-50/40"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-900">{patient.fullName}</p><p className="text-xs font-semibold text-slate-500">{patient.patientCode} · {patient.phone || 'Chưa có SĐT'}</p></div>{latestStatus ? <StatusBadge st={latestStatus} /> : <span className="text-xs font-bold text-slate-400">{visits.length} lượt</span>}</div><button type="button" onClick={onOpen} className="mt-3 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50">Chi tiết</button></article>; }
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
      ...
      <div className="relative flex max-h-[90vh] w-full max-w-[1280px] flex-col overflow-hidden rounded-2xl bg-white shadow-xl">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="mb-1 flex flex-wrap items-center gap-2">
                <span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-mono font-black text-slate-700">{patient.patientCode}</span>
                {latestStatus && <StatusBadge st={latestStatus} />}
              </div>
              <h2 className="text-2xl font-black text-slate-950">{patient.fullName}</h2>
            </div>
          </div>
          <button onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">×</button>
        </div>
        ...
        <div className="shrink-0 border-b border-slate-100 px-5 py-3">
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={`rounded-xl border px-4 py-2 text-xs font-black transition-colors ${activeTab === 'profile' ? 'border-cyan-600 bg-cyan-600 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
            >
              Hồ sơ bệnh nhân
            </button>
            <button
              ...
              className={`rounded-xl border px-4 py-2 text-xs font-black transition-colors ${activeTab === 'visits' ? 'border-cyan-600 bg-cyan-600 text-white shadow-sm' : 'border-slate-200 bg-white text-slate-600 hover:bg-slate-50'}`}
            >
              Lịch sử khám bệnh ({visits.length})
            </button>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto bg-slate-50/60 p-5">
          {activeTab === 'profile' ? (
            <section>
              <h3 className="mb-3 text-sm font-black uppercase tracking-wider text-slate-700">Thông tin bệnh nhân</h3>
              <dl className="divide-y divide-slate-100 rounded-2xl border border-slate-100 bg-white shadow-sm">
                {[
                  ...
                  ['CCCD', patient.citizenId || 'Chưa cập nhật'],
                  ['SĐT', patient.phone || 'Chưa cập nhật'],
                  ['BHYT', patient.insuranceNumber || 'Chưa cập nhật'],
                  ['Khẩn cấp', patient.emergencyContact || 'Chưa cập nhật'],
              ...
              <button onClick={onCreateVisit} className="mt-4 w-full rounded-xl bg-cyan-600 px-4 py-3 text-xs font-black text-white hover:bg-cyan-700">+ Tiếp nhận</button>
            </section>
          ) : (
            <section>
              <h3 className="mb-3 text-sm font-black uppercase tracking-wider text-slate-700">Lịch sử khám bệnh ({visits.length})</h3>
              <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
                <div className="divide-y divide-slate-100">
                  {visits.map((visit) => <VisitRow key={visit.id} visit={visit} />)}
                  {!visits.length && <Empty title="Chưa có lượt khám" compact />}
                </div>
              </div>
            </section>
          )}
