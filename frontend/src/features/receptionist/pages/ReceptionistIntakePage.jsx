...
export default function ReceptionistIntakePage() {
  ...
  return <DashboardLayout user={user} navItems={RECEPTIONIST_NAV_ITEMS} activeItem="patient-intake" onNavigate={(id) => navigate(receptionistRouteFor(id))} onLogout={logout}><div className="mx-auto max-w-6xl space-y-4"><section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm"><div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div><p className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-600">Tiếp nhận</p><h1 className="mt-1 text-2xl font-black text-slate-950">Tạo lượt</h1></div><StepBar selectedPatient={selectedPatient} /></div></section><section className="grid grid-cols-1 gap-4 lg:grid-cols-[330px_1fr]"><PatientSummary patient={selectedPatient} onClear={() => setSelectedPatient(null)} /><div className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm">{!selectedPatient ? <PatientFinder onPatientSelected={setSelectedPatient} /> : <CreateVisitForm patient={selectedPatient} onCancel={() => setSelectedPatient(null)} onVisitCreated={handleVisitCreated} />}</div></section></div></DashboardLayout>;
}
