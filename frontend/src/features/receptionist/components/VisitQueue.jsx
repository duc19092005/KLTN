...
function getVisitStaffName(visit) { return visit.staff?.fullName || visit.staff?.user?.username || 'Chưa rõ'; }
...
export default function VisitQueue({ refreshTrigger }) {
  ...
  const visibleVisits = useMemo(() => {
    ...
    return visits
      ...
      .filter((visit) => !text || [visit.visitCode, visit.patient?.fullName, visit.patient?.patientCode, visit.patient?.phone, getVisitDepartmentName(visit), getVisitStaffName(visit), getVisitStatus(visit.status).label].filter(Boolean).some((value) => String(value).toLowerCase().includes(text)))
      .sort((a, b) => {
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-100 bg-white shadow-sm">
        <div className="border-b border-slate-100 p-4 space-y-3">
          <div className="flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
            ...
            <button type="button" onClick={loadVisits} className="h-10 rounded-xl border border-slate-200 bg-white px-4 text-xs font-black text-slate-600 hover:bg-slate-50">{loading ? <LoadingIndicator size="sm" /> : 'Làm mới'}</button>
          </div>
...
function VisitRow({ visit, onOpen }) {
  const patient = visit.patient; const st = getVisitStatus(visit.status);
  return <tr onClick={onOpen} className="cursor-pointer bg-white hover:bg-cyan-50/40"><td className="px-5 py-4 min-w-[240px]"><div className="flex items-center gap-3"><div className="grid h-10 w-10 place-items-center rounded-2xl bg-cyan-50 text-xs font-black text-cyan-700">{patient?.fullName?.slice(0, 2).toUpperCase() || 'BN'}</div><div><p className="text-sm font-black text-slate-900">{patient?.fullName || 'N/A'}</p><p className="text-[11px] font-semibold text-slate-500">{patient?.patientCode || 'N/A'} · {patient?.phone || 'Chưa có SĐT'}</p></div></div></td><td className="px-5 py-4"><span className="rounded-lg bg-slate-100 px-2 py-1 text-[11px] font-mono font-black text-slate-600">{visit.visitCode}</span></td><td className="px-5 py-4 min-w-[220px]"><p className="text-xs font-black text-slate-800">{getVisitDepartmentName(visit)}</p><p className="text-[11px] font-semibold text-slate-500">BS. {getVisitStaffName(visit)}</p></td><td className="px-5 py-4"><StatusBadge st={st} /></td><td className="px-5 py-4 text-xs font-semibold text-slate-500">{visit.createdAt ? new Date(visit.createdAt).toLocaleString('vi-VN', { hour: '2-digit', minute: '2-digit', day: '2-digit', month: '2-digit' }) : 'N/A'}</td><td className="px-5 py-4 text-right"><button type="button" onClick={(event) => { event.stopPropagation(); onOpen(); }} className="rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50">Chi tiết</button></td></tr>;
}
function VisitCard({ visit, onOpen }) { const st = getVisitStatus(visit.status); return <button type="button" onClick={onOpen} className="w-full rounded-2xl border border-slate-100 bg-white p-4 text-left hover:bg-cyan-50/40"><div className="flex items-start justify-between gap-3"><div><p className="text-sm font-black text-slate-900">{visit.patient?.fullName || 'N/A'}</p><p className="text-xs font-semibold text-slate-500">{visit.visitCode} · {getVisitDepartmentName(visit)}</p></div><StatusBadge st={st} /></div></button>; }
function VisitDetailModal({ visit, busyId, onCancel, onClose }) { const st = getVisitStatus(visit.status); const canCancel = visit.status === 'WAITING'; useEffect(() => { document.body.style.overflow = 'hidden'; return () => { document.body.style.overflow = 'unset'; }; }, []); return <div className="fixed inset-0 z-[100] flex items-center justify-center p-4"><div className="absolute inset-0 bg-slate-900/60 backdrop-blur-sm" onClick={onClose} /><div className="relative w-full max-w-3xl overflow-hidden rounded-2xl bg-white shadow-xl"><div className="flex items-start justify-between gap-4 border-b border-slate-100 p-5"><div><div className="mb-2 flex flex-wrap items-center gap-2"><span className="rounded-lg bg-slate-100 px-2.5 py-1 text-xs font-mono font-black text-slate-700">{visit.visitCode}</span><StatusBadge st={st} /></div><h2 className="text-2xl font-black text-slate-950">{visit.patient?.fullName || 'N/A'}</h2><p className="mt-1 text-sm font-bold text-slate-500">{visit.patient?.patientCode || 'N/A'}</p></div><button onClick={onClose} className="grid h-10 w-10 place-items-center rounded-full bg-slate-100 text-slate-500 hover:bg-slate-200">×</button></div><div className="grid gap-3 p-5 sm:grid-cols-2"><Info label="SĐT" value={visit.patient?.phone || 'Chưa có'} /><Info label="Phòng" value={getVisitDepartmentName(visit)} /><Info label="Bác sĩ" value={`BS. ${getVisitStaffName(visit)}`} /><Info label="Giờ tạo" value={visit.createdAt ? new Date(visit.createdAt).toLocaleString('vi-VN') : 'N/A'} /></div>{canCancel && <div className="border-t border-slate-100 p-5 text-right"><button disabled={busyId === visit.id} onClick={() => onCancel(visit)} className="rounded-xl border border-rose-100 bg-rose-50 px-4 py-2.5 text-xs font-black text-rose-600 hover:bg-rose-100 disabled:opacity-50">{busyId === visit.id ? 'Đang hủy...' : 'Hủy lượt'}</button></div>}</div></div>; }
