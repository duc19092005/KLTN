# Blockchain-Based Verifiable Storage System for AI-Assisted Clinical Diagnostic Results
...
> 📌 **Project Scope Note:**
> The core research objective of this thesis is **the verifiable and tamper-evident storage of AI-assisted clinical diagnostic results using Blockchain and Merkle Trees**. Because standalone AI query flows represent a narrow data scope, the project modeled an expanded **streamlined hospital management workflow** (appointments, QR reception, lab orders, and clinician sign-offs) to establish a realistic end-to-end data lifecycle for the audit trail. This system serves as a testbed environment for research and evaluation rather than an enterprise replacement for actual full-scale hospital operations.
...
### ❓ Practical Challenge:
When integrating Artificial Intelligence (AI) into clinical decision support:
- AI models (e.g., Claude, GPT, Gemini) provide differential diagnostic suggestions and treatment options. Clinicians evaluate, adjust, or approve the final conclusion.
- **Core Problem:** If diagnostic results and AI consultation trails are stored solely in conventional centralized databases, records can be modified or deleted without leaving independent mathematical proof. In cases of clinical disputes or ethical audits, it is difficult to independently verify the exact suggestions generated by AI and the clinician's review actions at that time.
- **Requirement:** A tamper-evident storage mechanism capable of providing **non-repudiation and verifiable data authenticity** for both clinical conclusions and AI diagnostic logs, while strictly preserving patient privacy.

### 💡 Proposed Solution:
The system combines **Blockchain**, **IPFS**, and **Merkle Trees** to achieve:
1. **Verifiable Storage for AI & Clinical Conclusions:** Clinical events, AI recommendations, and practitioner sign-off decisions are cryptographically chained and anchored to on-chain checkpoints.
2. **Independent Verification (Merkle Proofs):** Third parties (patients, clinical review boards, insurers) can verify record authenticity against the blockchain without accessing internal database systems.
3. **Discrepancy Detection & Data Recovery (Self-Healing):** If database records are modified off-chain, the system identifies the altered record and facilitates restoration from IPFS.
...
|---|---|
| 💸 **Storage Cost (Gas Fees)** | On-chain storage is priced per word of state. Large medical files, diagnostic imaging, and detailed AI prompt histories incur significant overhead if stored directly on-chain. |
...
1. Full clinical audit payloads (including AI consultation logs) are maintained in the local database and archived in encrypted form on IPFS.
...
graph TD
    ...
    subgraph D["🏥 Clinical Data & AI Diagnostics (Hospital DB & Encrypted IPFS)"]
        DocA["📄 Episode 1: AI Diagnosis + Doctor Review (Patient A)"] --> H_A
        DocB["📄 Episode 2: Lab Order (Patient B)"] --> H_B
        DocC["📄 Episode 3: Lab Result (Patient C)"] --> H_C
        DocD["📄 Episode 4: Clinical Conclusion (Patient D)"] --> H_D
    end
...
4. Compare against the **on-chain Merkle Root**:
   - If **Match**: The clinical conclusion and AI review record are authentic and unaltered.
...
sequenceDiagram
    ...
    D->>API: Clinician Signs Medical Conclusion (with AI review trail)
