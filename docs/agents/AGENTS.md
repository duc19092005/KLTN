...
- **Database**: PostgreSQL.
- **ORM**: Prisma ORM (Canonical schema is at `apps/hospital-api/prisma/schema.prisma`).
...
**Patient Mobile:**
- **Framework**: Expo React Native in `apps/hospital-mobile/`.
...
- **Environment**: `EXPO_PUBLIC_BACKEND_URL` in `apps/hospital-mobile/.env`; never commit `.env`.
