...
GitHub Actions will sync these files into `/opt/kltn`:
...
```text
/opt/kltn/.env
/opt/kltn/backend/.env
/opt/kltn/frontend/.env
/opt/kltn/blockchain/.env
```

Create directories first:

```bash
mkdir -p /opt/kltn/backend /opt/kltn/frontend /opt/kltn/blockchain
```
...
```bash
nano /opt/kltn/backend/.env
chmod 600 /opt/kltn/backend/.env
```
...
- `backend/.env` must contain `DATABASE_URL` pointing to `postgres:5432`.
- `backend/.env` should use `KAFKA_ENABLED=true` and `KAFKA_BROKERS=kafka:9092`.
- `frontend/.env` values are public because `VITE_*` variables are built into the browser bundle.
...
docker compose -f docker-compose.prod.yml logs --tail=100 kafka
```
...
```cron
0 2 * * * cd /opt/kltn && ./scripts/backup-postgres.sh >> backups/backup.log 2>&1
```
...
```bash
cd /opt/kltn
./scripts/restore-postgres.sh backups/postgres/zkp_identity_YYYYMMDD_HHMMSS.sql.gz
```
...
Before deploy, the script records current backend/frontend image names in:
...
Replace `your-domain.com` in `nginx/conf.d/default.conf` with the real domain.
