...
> [!IMPORTANT]
> Tài liệu này phản ánh **cơ chế role hiện tại trong code** (`backend/prisma/schema.prisma` + các module NestJS),
...
Enum `UserRole` tại [schema.prisma](../prisma/schema.prisma) (dòng 10-16):
...
Khi gán trưởng phòng ([assign-manager.use-case.ts](../src/modules/department/application/use-cases/assign-manager.use-case.ts) và [create-department.use-case.ts](../src/modules/department/application/use-cases/create-department.use-case.ts)), role của trưởng phải khớp loại phòng ban:
...
Kiểm tra thô theo `user.role`. Logic cực kỳ đơn giản ([roles.guard.ts](../src/modules/auth/guards/roles.guard.ts)):

```ts
return Boolean(user?.role && requiredRoles.includes(user.role));
```

Khai báo trên controller/handler ([roles.decorator.ts](../src/common/decorators/roles.decorator.ts)):
...
Quyền "chỉ trưởng phòng ban mới được làm" được kiểm tra **bên trong use-case**, truy DB qua quan hệ `manager`. Ví dụ duyệt ca trực ([approve-reception-shift.use-case.ts](../src/modules/reception-shift/application/use-cases/approve-reception-shift.use-case.ts), [approve-shift.use-case.ts](../src/modules/paraclinical-shift/application/use-cases/approve-shift.use-case.ts)):
...
Các use-case lọc dữ liệu theo phòng ban đang quản lý dùng `StaffProfile.managedDepartment` (xem [list-pending-shifts.use-case.ts](../src/modules/paraclinical-shift/application/use-cases/list-pending-shifts.use-case.ts)).
...
- **Tự động tạo** khi tạo phòng ban ([create-department.use-case.ts](../src/modules/department/application/use-cases/create-department.use-case.ts)).
...
1. **Pha 1 — mật khẩu** ([paraclinical-login.use-case.ts](../src/modules/paraclinical-shift/application/use-cases/paraclinical-login.use-case.ts)): xác thực username/password của tài khoản chung → cấp **tempToken** giới hạn (chỉ cho phép bước quét mặt, KHÔNG có quyền sâu).
2. **Pha 2 — quét mặt** ([verify-shift-face.use-case.ts](../src/modules/paraclinical-shift/application/use-cases/verify-shift-face.use-case.ts)): khuôn mặt xác định **con người thật** đang trực ca, gắn danh tính cá nhân vào phiên làm việc.
...
Khi xếp một nhân viên vào phòng ban ([staff.validator.ts](../src/modules/staff/application/services/staff.validator.ts) `assertDepartmentRoleCompatible`):
...
Payload gắn vào `req.user` bởi [jwt.strategy.ts](../src/modules/auth/strategies/jwt.strategy.ts), kiểu [auth-user.type.ts](../src/common/types/auth-user.type.ts):
